...
public class RoleMixinPattern extends ClassSelectionPanel {
	...
	public RoleMixinPattern(OntoUMLParser parser) {
		...
		roleMixinPanel.setBounds(0, 274, 435, 60);
		add(roleMixinPanel);
		roleMixinPanel.setLayout(null);
		
		lblRoleMixin = new JLabel("RoleMixin0:");
	...
	protected void getModelValues(OntoUMLParser parser) {
		...
		if(roleMixins.size() > 0){
			String[] roleMixinVector = UtilAssistant.getStringRepresentationClassStereotype(roleMixins);
			model = new DefaultComboBoxModel<String>(roleMixinVector);  
