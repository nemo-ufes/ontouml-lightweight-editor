 ...
 *        annotation="http://www.eclipse.org/ocl/examples/OCL sourceIsParticipant_Reference='self.source.oclIsKindOf(EntityParticipant) or self.source.oclIsKindOf(RelatorParticipant) or (self.source.oclIsKindOf(ReferenceNode) and (self.source.oclAsType(ReferenceNode).reference.oclIsKindOf(EntityParticipant) or self.source.oclAsType(ReferenceNode).reference.oclIsKindOf(RelatorParticipant)))' targetIsLiteral='self.target.oclIsKindOf(Literal) and self.target.oclAsType(Literal).type.oclIsKindOf(RefOntoUML::Class)'"
 *        annotation="Comments sourceIsParticipant_Reference='The source of an Instantiation must be an EntityParticipant, a RelatorParticipant or a ReferenceNode, which in this case must be a reference to an EntityParticipant or RelatorParticipant' targetIsLiteral='The target of an Instantiation must be a Literal and its type must be a RefOntoUML::Class'"
 * @generated
 */
