 ...
 *
 */
public class CompleteSelectionDialog extends JDialog {
	...
	public static void open(TheFrame frame) 
	{
		try {
			...
			CompleteSelectionDialog dialog = new CompleteSelectionDialog(frame);
	 ...
	 * @param frame
	 */
	public CompleteSelectionDialog(TheFrame frame)
	{
		this();
	...
	public void AutoSelectionActionPerformed(ActionEvent event)
	{
		if (rbAllAncestors.isSelected())
		{
			if (frame.getManager().getOntoUMLView().getModelTree()==null) return;       		    	
		   	String msg = frame.getManager().UpdateSelection(OntoUMLParser.ALL_ANCESTORS);       	   				    	
	   		JOptionPane.showMessageDialog(
	   			frame,msg,"All ancestors",JOptionPane.INFORMATION_MESSAGE,
	   			new ImageIcon(OCLEditorBar.class.getResource("/resources/br/ufes/inf/nemo/move/autoselection-36x36.png"))
	   		); 
		}
		if (rbAllAncestorsDescendants.isSelected())
		{
			if (frame.getManager().getOntoUMLView().getModelTree()==null) return;       		    	
		   	String msg = frame.getManager().UpdateSelection(OntoUMLParser.COMPLETE_HIERARCHY);       	   				    	
	   		JOptionPane.showMessageDialog(
		...
		{
			if (frame.getManager().getOntoUMLView().getModelTree()==null) return;       		    	
		   	String msg = frame.getManager().UpdateSelection(OntoUMLParser.SORTAL_ANCESTORS);       	   				    	
	   		JOptionPane.showMessageDialog(
		...
		{
			if (frame.getManager().getOntoUMLView().getModelTree()==null) return;       		    	
		   	String msg = frame.getManager().UpdateSelection(OntoUMLParser.ALL_DESCENDANTS);       	   				    	
	   		JOptionPane.showMessageDialog(
		...
		{
			if (frame.getManager().getOntoUMLView().getModelTree()==null) return;       		    	
		   	String msg = frame.getManager().UpdateSelection(OntoUMLParser.NO_HIERARCHY);       	   				    	
	   		JOptionPane.showMessageDialog(
	 ...
	 * Create the dialog.
	 */
	public CompleteSelectionDialog() 
	{
		setIconImage(Toolkit.getDefaultToolkit().getImage(CompleteSelectionDialog.class.getResource("/resources/br/ufes/inf/nemo/move/autoselection-16x16.png")));
