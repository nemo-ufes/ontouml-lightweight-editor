...
import br.ufes.inf.nemo.move.util.ui.CheckBoxListScrollPane;

/**
 * @author John Guerson
 */

public class IAAntiPatternView extends JPanel {
	 ...
	 * @param frame
	 */
	public IAAntiPatternView(IAAntiPatternModel iaModel,TheFrame frame)
	{
		this();
		...
		textAssociation.setText(iaModel.getIAAntiPattern().getAssociation().getName());
		textSource.setText(iaModel.getIAAntiPattern().getSource().getName());
		textTarget.setText(iaModel.getIAAntiPattern().getTarget().getName());
		
		if (iaModel.getIAAntiPattern().getSourceChildren().size()==0)
		{
			checkListSource.setEnabled(false);
			cbxSourceCustom.setEnabled(false);
		}else{		
			for(Classifier c: iaModel.getIAAntiPattern().getSourceChildren())
			{
		...
		if (iaModel.getIAAntiPattern().getTargetChildren().size()==0)
		{
			checkListTarget.setEnabled(false);
			cbxTargetCustom.setEnabled(false);
		}else{				
			for(Classifier c: iaModel.getIAAntiPattern().getTargetChildren())
			{
				JCheckBox cbx = new JCheckBox(c.getName());
				cbx.setBackground(Color.WHITE);
				cbx.setHorizontalAlignment(SwingConstants.LEFT);
		...
		lblImpreciseAbstraction.setText("Imprecise Abstraction "+iaModel.getId());
	}	
	
	/**
	 * Constructor.
	 */
	public IAAntiPatternView() 
	{
