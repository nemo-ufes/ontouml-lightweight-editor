...
public class RelCompFirstPage extends RelCompPage {
	...
	public void createControl(Composite parent) {
		...
		lblQuestion.setBounds(10, 10, 554, 40);
		...
		btnSourceDependsOnTarget.setBounds(10, 56, 554, 16);
		btnSourceDependsOnTarget.addSelectionListener(listener);
		btnSourceDependsOnTarget.setText("'x' be connected to all 'w' (instances of '') that 'y' is connected through ''");	
		
		btnTargetDependsOnSource = new Button(container, SWT.RADIO);
		btnTargetDependsOnSource.setBounds(10, 78, 554, 16);		
		btnTargetDependsOnSource.addSelectionListener(listener);
		btnTargetDependsOnSource.setText("'y' be connected to all 'w' (instances of '') that 'x' is connected through ''");	
		
		if(getRelComp().a2EndsSpecializeA1Target()){
			btnSourceDependsOnTarget.setText("'x' be connected to all 'w' (instances of '"+getRelComp().getA1Source().getName()+"') that 'y' is connected through '"+
											 getRelComp().getParser().getStringRepresentation(getRelComp().getA1())+"'");	
			btnTargetDependsOnSource.setText("'y' be connected to all 'w' (instances of '"+getRelComp().getA1Source().getName()+"') that 'x' is connected through '"+
											 getRelComp().getParser().getStringRepresentation(getRelComp().getA1())+"'");	
		}
		else{
			btnSourceDependsOnTarget.setText("'x' be connected to all 'w' (instances of '"+getRelComp().getA1Target().getName()+"') that 'y' is connected through '"+
					 getRelComp().getParser().getStringRepresentation(getRelComp().getA1())+"'");
			btnTargetDependsOnSource.setText("'y' be connected to all 'w' (instances of '"+getRelComp().getA1Target().getName()+"') that 'x' is connected through '"+
					 getRelComp().getParser().getStringRepresentation(getRelComp().getA1())+"'");	
		}
		
		btnBoth = new Button(container, SWT.RADIO);
		btnBoth.setText("First two options are true");
		btnBoth.setBounds(10, 100, 554, 16);
		...
		btnNone.setBounds(10, 122, 554, 16);
