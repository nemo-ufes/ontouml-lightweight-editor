...
public class UMLSettingDialog extends JDialog {
	...
	{
		try {			
			UMLSettingDialog dialog = new UMLSettingDialog(parent,modal,refparser);
	...
	public UMLSettingDialog(final JFrame parent, boolean modal, final OntoUMLParser refparser) 
	{
		...
		txtpnThisIsA.setText("This transformation to UML ignores all the stereotypes of classes and relationships of OntoUML. Check the options you want to enforce and then press the button to transform your OntoUML model into UML.");
		...
		pathField.setText(userhome+File.separator+"Model.uml");
		
		btnBrowse = new JButton("...");
		btnBrowse.addActionListener(new ActionListener() {					
			@Override
			public void actionPerformed(ActionEvent arg0) {				
				FileNameExtensionFilter filter = new FileNameExtensionFilter("UML Model (*.uml)", "uml");
				fileChooser.setDialogTitle("Transformation -> UML -> Browse");
				fileChooser.addChoosableFileFilter(filter);
				fileChooser.setFileFilter(filter);
				fileChooser.setSelectedFile(new File("Model.uml"));
				fileChooser.setAcceptAllFileFilterUsed(false);
				if (fileChooser.showDialog(UMLSettingDialog.this,"OK") == JFileChooser.APPROVE_OPTION) {
					try {						
						File file = fileChooser.getSelectedFile();				
						if (!file.exists()) {
							if(!file.getName().endsWith(".uml")) {
								file = new File(file.getCanonicalFile() + ".uml");
							}else{
